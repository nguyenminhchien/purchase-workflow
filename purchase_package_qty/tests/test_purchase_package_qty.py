...
class TestPurchasePackageQty(AccountTestInvoicingCommon):
    ...
    def test_000_product_supplier_info_package(self):
        ...
        with product_form.variant_seller_ids.new() as psi:
            ...
            self.assertEqual(psi.price, 19)
            self.assertEqual(psi.base_price, 19 * 12)
    ...
    def test_001_purchasing_flow(self):
        ...
        self.assertEqual(self.po_line_1.price_unit, 20)
        ...
        with po_form.order_line.edit(1) as line_form:
            ...
            self.assertEqual(line_form.price_subtotal, 17 * 12 * 2)
            self.assertEqual(line_form.product_packaging_id, self.packaging_dozen)
            self.assertTrue(line_form.indicative_package)

            # Update the package price directly from the PO Line
            line_form.price_unit = 18 * 12  # update from 17 to 18
            self.assertEqual(line_form.price_subtotal, 18 * 12 * 2)

        with po_form.order_line.edit(0) as line_form:
            line_form.price_policy = "package"
            # Test subtotal updated as well
            self.assertEqual(line_form.price_subtotal, 20 * 1)  # 1 package
            line_form.price_policy = "uom"
            self.assertEqual(line_form.price_subtotal, 20 * 5)  # 5 product qty
        ...
        amount_line2 = 2 * (18 * 12)  # package qty*price (price_policy=package)
        ...
        for stock_move, idx in zip(picking.move_ids, [0, 1], strict=False):
            ...
            self.assertEqual(
                stock_move.product_packaging_id, po_line.product_packaging_id
            )
            self.assertEqual(
                stock_move.product_packaging_qty, po_line.product_packaging_qty
            )
            self.assertEqual(
                stock_move.product_packaging_quantity, po_line.product_packaging_qty
            )
        ...
        for invoice_line, idx in zip(invoice.invoice_line_ids, [0, 1], strict=False):
            po_line = self.purchase_order.order_line[idx]
            self.assertEqual(
                invoice_line.product_packaging_id, po_line.product_packaging_id
            )
            self.assertEqual(
                invoice_line.product_packaging_quantity, po_line.product_packaging_qty
            )
            ...
            if idx == 0:
                ...
                # price_policy = package
                self.assertEqual(invoice_line.quantity, po_line.product_packaging_qty)
    ...
    def test_002_manual_receipt(self):
        ...
        with picking_form.move_ids_without_package.new() as move:
            ...
            move.product_packaging_id = self.packaging_dozen
            move.product_packaging_qty = 3
            self.assertEqual(move.product_uom_qty, 3 * 12)
            self.assertEqual(move.product_packaging_quantity, 0)
        picking = picking_form.save()
        picking.action_confirm()
        self.assertEqual(
            picking.move_ids_without_package[0].product_packaging_quantity, 3
        )
        picking.move_ids_without_package[0].product_packaging_quantity = 2
        self.assertEqual(picking.move_ids_without_package[0].quantity, 2 * 12)
    ...
    def test_003_manual_invoice(self):
        ...
        with bill_form.invoice_line_ids.new() as line:
            ...
            line.product_packaging_id = self.packaging_dozen
            line.product_packaging_quantity = 3
